...
In the callback, the `Response` object is supplied with `statusCode` and `payload`. Fields on the payload are accessible via the `com.apama.util.AnyExtractor` object it is supplied in - see the information on [access fragments](/apama/actions/#utility-functions) below.
