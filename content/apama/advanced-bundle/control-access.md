...
available within the tenant, then the relevant part is disabled. This can be done either within a tenant or by an {{< tenant-type-2 >}} or {{< tenant-type-3 >}}
(see also [{{< tenant-type-2 >}} > Managing tenants](/users-guide/enterprise-edition/#managing-tenants) in the *User guide*)
...
(also see the [Tenant API](https://{{< URL >}}/api/#tag/Tenant-API) in the {{< OpenAPI >}}) or use a curl command as given in the example below:
