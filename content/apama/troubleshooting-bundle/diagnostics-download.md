...
- The microservice log file contents, if available, including a record of the correlator's startup logging and the last hour or maximum of 20,000 lines of logging.
    ...
    > **Info:** In case of {{< product-c8y-iot >}} Edge 10.6.0, since Apama is not a microservice, Apama logs can be retrieved using the diagnostic utility. For more details, see [Apama log file locations](/edge/operation/#apama-log-file-locations) and [Diagnostics](/edge/operation/#diagnostics) both in the *{{< product-c8y-iot >}} Edge* guide.
