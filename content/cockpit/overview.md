...
{{< c8y-admon-info >}}
For performance reasons, the map on the Home dashboard displays icons a maximum of 100 devices. If there are more devices they are not shown in the map. To see them, you must go to the dashboard on group level, and add the "Map" widget there to only see devices from this particular group, see [Widgets collection](#widgets-collection).
{{< /c8y-admon-info >}}
