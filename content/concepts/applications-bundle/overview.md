...
Applications regardless of its form are identified by a so-called *application key*. The application key enables Cumulocity to associate a REST request from an application with the particular application, see the section on application management in the [Reference guide](/guides/reference/applications).
