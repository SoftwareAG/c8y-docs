...
|Cloud|When configuring an Azure Storage data lake in the settings page, an invalid Azure Storage account name is not handled properly. The same holds for an invalid AWS access key for an Amazon S3 data lake. When saving the settings with such an invalid value, the backend system will run a time-consuming check, which shows up as ongoing save request in the UI. Eventually the request will fail in the UI with a time-out and the save request in the backend will fail as well. In such a case you have to check your data lake settings and save them again.|