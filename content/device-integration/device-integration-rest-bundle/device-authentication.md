...
Replace `<device certificate chain>` with your valid certificate chain when registering with {{< product-c8y-iot >}}. Header `<device certificate chain>` is not mandatory if we have immediate issuer of device certificate in {{< product-c8y-iot >}}.
