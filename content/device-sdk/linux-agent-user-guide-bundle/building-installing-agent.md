...
### <a name = building-the-cumulocity-cloud-remote-access-service>Building the Cumulocity Cloud Remote Access Service </a>
...
Modbus support is disabled by default. In between step 4 and step 5 of [Building the basic agent](#basic-agent), you need to do one additional step. The Modbus feature requires `libmodbus` library, so that make sure you have `libdmobus` installed before building with Modbus support.
...
### <a name= building-the-cumulocity-canopen-service> Building the Cumulocity CANopen Service </a>
...
> **Note:** Before you install the agent, you need to configure the agent parameters in _cumulocity-agent.conf_ file. Please refer to [Configuring the agent](#configuring-agent) Section.
