...
The MQTT implementation of {{< product-name-1 >}} is specifically designed for device communication. Therefore, it tries to remove as much unnecessary logic from the client side as possible.
...
To remove the necessity of persisting the ID on the device, {{< product-name-1 >}} offers the identity API where you can link external IDs (e.g. a serial number) to the object, so you can query the ID any time.
...
When a client creates an alarm using the REST API, it needs to ensure that it gets the ID of the alarm that was generated by {{< product-name-1 >}} in return.
...
In {{< product-name-1 >}}, a device can only have a single alarm per type in status ACTIVE. If it creates another alarm with the same type, it will get de-duplicated.
