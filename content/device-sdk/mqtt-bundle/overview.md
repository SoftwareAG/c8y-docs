...
The MQTT implementation of {{< product-name-1 >}} provides the following benefits:
...
* Device registration: Non-personalized devices can be deployed by pairing them with {{< product-name-1 >}} tenants.
...
* Processing modes: Control whether data is persisted in {{< product-name-1 >}} database, transiently passed to real-time processing, processed using quiescent mode which ensures that real-time notifications are disabled or is processed using CEP mode that ensures data is transiently sent to real-time processing engine only with real-time notifications disabled.
...
* [MQTT implementation](/device-sdk/mqtt#implementation) gives a detailed reference of protocol-level aspects in the {{< product-name-1 >}} implementation of MQTT.
* [Device integration](/device-sdk/mqtt#device-integration) walks you through the process of interfacing devices with {{< product-name-1 >}} through MQTT.
...
* [Handling IDs](/device-sdk/mqtt#handling-ids) describes how IDs are handled in the {{< product-name-1 >}} MQTT protocol.
* [JSON via MQTT](/device-sdk/mqtt/#json) describes the payload format that can be used with the {{< product-name-1 >}} MQTT implementation.
