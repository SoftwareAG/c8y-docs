...
|tlsSecretName|No|string||The name of the Kubernetes secret containing the TLS key and the certificate chain with which Edge connects to the cloud through MQTT protocol using a X.509 certificate for authentication. For more information, see [TLS secret](/edge-kubernetes/edge-custom-resource-definition/#tls-secret) for details.<p><p>{{< c8y-admon-info >}} The Edge operator retrieves this secret from the **`EDGE-CR-NAMESPACE`**. Ensure that this secret is created before initiating the Edge deployment or update process. {{< /c8y-admon-info >}}
