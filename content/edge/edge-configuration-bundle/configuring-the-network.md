...
Configuring the network enables you to access the {{< product-c8y-iot >}} Edge appliance using the domain name. If there are any changes in the network configuration due to an update or changes in the network protocols, you must reconfigure the network of your {{< product-c8y-iot >}} Edge appliance as described below. After reconfiguring the network, you must update the IP mapping for the domain name with the new IP address in the DNS servers, if you have added an entry in the DNS servers. If you are using an alias, you must update the IP address in the hosts file of your operating system.
...
3. Click **Edge** > **Network** in the navigator.
   ...
   The current network configuration for the {{< product-c8y-iot >}} Edge appliance appears.
...
9. Provide the **Address range CIDR**. For example, 172.18.0.0/16
   ...
   The **Address range CIDR** must not overlap with the reserved IP addresses. See [Reserved IP addresses](https://en.wikipedia.org/wiki/Reserved_IP_addresses).
  	...
  	{{< c8y-admon-info >}}
If the IP address of the {{< product-c8y-iot >}} Edge appliance overlaps with the appliance's address range, then you must update the **Address range CIDR**.
		{{< /c8y-admon-info >}}
...
2. Invalid network CIDR provided during IP change.
	...
	Description: You tried to change the network CIDR of the properly running {{< product-c8y-iot >}} Edge appliance, but the range is not available in the network.

To troubleshoot these scenarios:

1. Configure the {{< product-c8y-iot >}} Edge appliance's network and gateway to a different network range. Configuring the {{< product-c8y-iot >}} Edge appliance to a different network disconnects the {{< product-c8y-iot >}} Edge appliance from the existing network.
2. Configure the network CIDR to a new non-conflicting value.
3. Configure the {{< product-c8y-iot >}} Edge appliance's network and gateway back to the network range.
