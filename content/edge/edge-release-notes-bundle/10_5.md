...
You can now register and manage your Cumulocity IoT Edge VM remotely from your Cumulocity IoT tenant. You must first configure the Edge agent (see [Configuring Edge agent](/guides/edge/installation/#configuring-edge-agent)) and then register the Edge VM with the Cumulocity IoT tenant (see [Registering the Edge device with a Cumulocity IoT tenant](/guides/edge/usage/#registering-the-edge-device-with-the-cumulocity-tenant)).
