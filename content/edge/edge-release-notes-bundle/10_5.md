...
You can now register and manage your Cumulocity IoT Edge device remotely from your Cumulocity IoT tenant. You must first configure the Edge agent (see [Configuring Edge agent](/edge/installation/#option-7-configure-edge-agent)) and then register the Edge device with the Cumulocity IoT tenant (see [Registering the Edge device with a Cumulocity IoT tenant](/edge/usage/#registering-the-edge-device-with-the-cumulocity-tenant)).
