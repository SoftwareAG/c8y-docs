...
Microservices deployed on the platform have a specific runtime environment and they need to understand certain details about the specific {{< product-name-1 >}} cluster they run in. For example, a microservice needs to know the endpoint address of the {{< product-name-1 >}} REST APIs. This information is provided by environment variables and they are injected by {{< product-name-1 >}} when the container is started.
...
To execute requests against the {{< product-name-1 >}} platform running a microservice, you have to send requests to the host specified by the `C8Y_BASEURL` variable.
