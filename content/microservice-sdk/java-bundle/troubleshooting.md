...
You can use both HTTP and HTTPS from the Java client libraries. To use HTTPS, you may need to import the {{< product-name-1 >}} production certificate into your Java Runtime Environment. Download the certificate with the following command:
...
Check if you are using the correct target platform. Go to the **Target Platform** page in the preferences and check if "{{< company-name-1 >}} runtime" is checked.
