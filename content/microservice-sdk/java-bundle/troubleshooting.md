...
#### I get "Expected to find an object at table index" when running an agent or application
...
Verify that all required plug-ins are checked in your launch configuration. Go to **Run** > **Run Configurations** and select the **OSGi Framework** launch configuration. Click **Select All** and try running again.
...
so the process 12985 is using that port which can then be killed if necessary.
