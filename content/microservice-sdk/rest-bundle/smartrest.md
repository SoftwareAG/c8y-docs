...
The image below illustrates how SmartREST works. Devices and other clients connect to a dedicated SmartREST endpoint on {{< product-name-1 >}} and send their data in rows of comma-separated values. These rows are expanded by {{< product-name-1 >}}'s SmartREST proxy into standard {{< product-name-1 >}} REST API requests. In the same way, responses from {{< product-name-1 >}} are compressed by the proxy from their original JSON format into comma-separated values before sending them back to the device.
...
* A request body contains rows of text in comma-separated value format. Each row corresponds to one request to the standard {{< product-name-1 >}} REST API.
* The response is always "200 OK".
* The response body again contains rows of comma-separated values. A row corresponds to a response from the {{< product-name-1 >}} REST API on a particular request.
...
The above example illustrated the handling of requests and request templates. For responses, [JSONPath](http://goessner.net/articles/JsonPath/) expressions translate {{< product-name-1 >}} REST responses into CSV. Assume, for example, that a device has a display and can show a message on it. An operation to update the message would look like:
...
A device usually queries for all operations that are associated with it and that have a status of PENDING. The standard {{< product-name-1 >}} response to such queries looks like:
