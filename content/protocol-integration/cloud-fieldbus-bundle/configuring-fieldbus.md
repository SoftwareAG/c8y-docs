...
2. In the **Value selection** section, enter the number of the register in the Modbus device. You can indicate a subset of bits to be used from a register by providing a start bit and a number of bits. This allows you to split a physical Modbus register into a set of "logical registers". It is important to note that in Cumulocity IoT registers are numbered as per the standard Modbus specification i.e. they start from 1. This differs with some device manufacturers which count registers starting from 0.
