...
In this repository you will find an example for the lansitec codec (`lora-codec-lansitec`). It is implemented using Spring Boot.
...
3. Implement the `DecoderService` interface.
    ...
    A flexible option named `success` is provided in the DecoderResult which indicates whether the `decode` operation was successful or not.

4. Implement the `EncoderService` interface.
...
5. Add the following permissions in the _cumulocity.json_ file. It looks like this:
