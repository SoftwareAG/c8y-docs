...
> **Info:** You can connect any device supporting LWM2M 1.0 to {{< product-name-1 >}} without programming. We expect the device and its capabilities (e.g. firmware update) to be compliant to the LWM2M specification. The device must support the UDP binding of the LWM2M standard.
...
For arbitrary protocols, you can configure how LWM2M devices are mapped to {{< product-name-1 >}} using [device protocols](/protocol-integration/cloud-fieldbus/#configuring-fieldbus).
