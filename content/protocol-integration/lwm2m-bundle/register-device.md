...
This property enables {{< product-c8y-iot >}} to generate a bootstrap server configuration (for example the security information like server URI, server ID, security mode, and so on) for the LWM2M bootstrap server and writes it back to the client during bootstrap. Default is false.
