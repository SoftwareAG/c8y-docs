...
1. When a shell command is created the corresponding operation status is set to ```PENDING```. This means that the corresponding CoAP request has not been yet sent to the device.
2. {{< product-c8y-iot >}} processes the shell command. It sends a corresponding CoAP request to the device and updates the operation status to ```EXECUTING```.
