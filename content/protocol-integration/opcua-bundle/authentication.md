...
{{< c8y-admon-info >}}
Beside the above authentication certificate, the device gateway also automatically creates a so-called web app identity certificate to identify itself with the OPC UA server. This needs to be trusted by the OPC UA server as well.
{{< /c8y-admon-info >}}
