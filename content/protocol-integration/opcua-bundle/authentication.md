...
In case a node is found with a non-existing name space entry the scan process skips this node and continues.
An error entry is written to the log file with information on how to fix the issue. Set the log level to DEBUG to add more information.
