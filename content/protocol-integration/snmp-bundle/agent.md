...
The SNMP agent is a stand-alone Java program that communicates with SNMP-enabled device(s) and the {{< product-name-1 >}} platform. It receives SNMP data from the devices, converts the data to {{< product-name-1 >}}-based objects based on the device protocol mapping, persists the data locally, and forwards the data to {{< product-name-1 >}}. The agent has to be registered in {{< product-name-1 >}} before serving the device request.
...
| Disk space    | The {{< product-name-1 >}} representation of the SNMP message will be persisted before forwarding to the platform. <br>Based on the load, sufficient disk space should be available to store the objects.     |
...
5. Configure the agent:
   ...
   * Change the properties according to the {{< product-name-1 >}} environment (e.g. gateway.identifier, {{< product-name-1 >}} bootstrap details, SNMP Community target).
...
7. Delete the snmp-agent device in {{< product-name-1 >}} which was registered as part of the installation and all its child SNMP devices. This can be done from the user interface or by using REST endpoints.
