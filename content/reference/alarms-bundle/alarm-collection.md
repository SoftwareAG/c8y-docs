...
**Alarm suppression** If the source device is in [maintenance](/reference/device-management) mode, the alarm is not created and not reported to the Cumulocity IoT event processing engine. After sending a POST request to create a new alarm and the device is in maintenance mode the self link of the alarm will be:
