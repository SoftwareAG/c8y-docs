...
**Alarm suppression** If the source device is in [maintenance](/guides/reference/device-management) mode, the alarm is not created and not reported to the Cumulocity event processing engine. After sending a POST request to create a new alarm and the device is in maintenance mode the self link of the alarm will be:
