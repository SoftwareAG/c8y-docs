...
The [Notifications 2.0 API](https://{{< domain-c8y >}}/api/{{< c8y-current-version >}}/#tag/Notification-2.0-API) allows web apps or microservices to receive and process notifications generated by the use of the {{< product-c8y-iot >}} REST or MQTT APIs (device management, measurements, alarms, events and other platform APIs) in a reliable manner.
...
To receive notifications over the Notifications 2.0 protocol, a web app or microservice must subscribe to notifications, either for notifications about a particular managed object or in a wider context that is scoped to a tenant. Subscriptions are persistent, long lived, and allow notifications to be stored reliably until consumed and acknowledged by a consuming microservice or web app.
...
In order to receive subscribed notifications, a consumer web app or microservice must obtain an authorization token that provides proof that the holder is allowed to receive subscribed notifications.
...
The underlying Messaging Service will repeatedly attempt to deliver a notification until that notification is acknowledged as being received and processed by the consuming web app or microservice.
...
This allows a web app to discover a new managed object, which can then choose to subscribe to in the managed object context.
...
### Building consuming microservices and web apps

For Java developers, the API and the protocol have been wrapped up as an open Java API and a sample WebSocket client web app. Any WebSocket library or programming language can be used as the protocol is text-based and relatively simple. Consumer can be either microservices or web apps running externally from {{< product-c8y-iot >}} and require only a JWT string when connecting.
...
In order to scale, shared subscriptions are required so that notifications are dispatched to one of a number of possible consumers that are part of the same logical subscriber or parallelized web app.
This can be achieved by creating a token with a subscription and subscriber name for the scalable web app with the optional boolean `shared` request parameter set to true in the token create request.
...
The subscription name defines the topic messages are published on, while the subscriber identifies the backend (north side) web app that will consume the notifications.
The web app can consist of more than one instances or "consumers" running in parallel in the shared use case.

Notifications will be delivered in order with respect to the notification generating device.
The notifications will be delivered to the same web app instance, except when a new instance or a failure changes the web app topology.
...
Currently, determining the instance ID of a microservice replica is not supported by the {{< product-c8y-iot >}} microservice API. An external web app with named instances can be used instead.
...
After the subscription is deleted no more notifications will be saved. The consuming microservice or web app can then drain down notifications 
...
For persistent subscriptions, this can result in notifications remaining in storage if never consumed by the web app.
However, unconsumed persistent notifications will be retained, and for high throughput scenarios this can result in notifications remaining in storage if never consumed by the web app.
...
However, unsubscribing a microservice or a web app is an explicit act using the original or a similar token.
Unsubscribing should be infrequent, for example when deleting a web app or during development when testing completes,
