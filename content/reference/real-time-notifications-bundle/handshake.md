...
> **Info** The number of parallel connections that can be opened at the same time by single user is limited. After exceeding this limit when a new connection is created, the oldest one will be closed and the newly created one will be added in its place.
