 ...
 * Days are counted according to server timezone so please be aware tenant usage statistics displaying/filtering may not work correctly when client is not in the same timezone as server. However, it is possible to send request with time range (`dateFrom` and `dateTo`) in zoned format (e.g. `2020-10-26T03:00:00%2B01:00`).
