...
When setting up an offloading pipeline for a Cumulocity IoT collection, DataHub automatically selects a set of default attributes to offload. Additional top-level attributes in the collection can be included in the offloading by defining so-called additional result columns. DataHub now facilitates the definition of those additional result columns by auto-detecting top-level attributes and guiding the user in the configuration process. 
...
Besides English and German, DataHub supports with this release an extended set of languages, including for example Spanish, French, Japanese, and others.
