...
You can now register and manage your Cumulocity IoT Edge VM remotely from your Cumulocity IoT tenant. You must first configure the Edge agent (see [Configuring Edge agent](https://cumulocity.com/guides/10.5.0/edge/installation/#configuring-edge-agent)) and then register the Edge VM with the Cumulocity IoT tenant (see [Registering the Edge device with a Cumulocity IoT tenant](https:/cumulocity.com/guides/10.5.0//edge/usage/#registering-the-edge-device-with-the-cumulocity-tenant)).
...
* If you are upgrading to Cumulocity IoT Edge version 10.5 from any previous version, note that you must redeploy all of your EPL files. See [Apama streaming analytics - New code editor and manager](/release-10-4-6/streaming-analytics/#apama-streaming-analytics-new-code-editor-and-manager).
