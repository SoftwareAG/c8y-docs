...
|CIE-2331|Administration|**Issue:** Switching the Edge agent URL fails as the device credentials are cached. This causes the registration to fail when connected to another Cumulocity IoT tenant URL.<br><br>**Workaround:** Delete the credentials from the location */usr/edge/properties/edge-agent/workspace/storage/*. Also, delete these credentials when you disable remote-connectivity.
