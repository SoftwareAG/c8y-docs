---
weight: 120
title: Release 10.4.0.15 - 10.4.0.16
layout: redirect
---

### Fixes

<table>
<colgroup><col width="150">
</colgroup><thead>
<tr>
<th style="text-align:left">Area</th>
<th style="text-align:left">Description</th>
<th style="text-align:left">Jira ticket</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left">Enterprise Tenant</td>
<td style="text-align:left">Enterprise Tenants can now also unsubscribe applications for subtenants that are not subscribed by the Enterprise Tenant itself.</td>
<td style="text-align:left">MTM-25288</td>
</tr>
<tr>
<td style="text-align:left">UI / Device Management</td>
<td style="text-align:left">The date picker is no longer cut off in the Tracking tab of a device.</td>
<td style="text-align:left">MTM-26204</td>
</tr>
<tr>
<td style="text-align:left">Microservices</td>
<td style="text-align:left">CellId no longer fails with “AbstractMethodError” on startup.</td>
<td style="text-align:left">MTM-26843</td>
</tr>
<tr>
<td style="text-align:left">Microservices</td>
<td style="text-align:left">Time stamps in the microservice status tab are now correct and consistent.</td>
<td style="text-align:left">MTM-26202</td>
</tr>
<tr>
<td style="text-align:left">REST</td>
<td style="text-align:left">The correct “self” link is now shown when executing “{{url}}/user/inventoryroles”.
</td>
<td style="text-align:left">MTM-25213</td>
</tr>
</tbody>
</table>






