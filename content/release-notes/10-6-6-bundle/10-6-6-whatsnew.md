...
For details on creating new widgets based on Angular, see [How-to recipes > Add a custom widget to a dashboard](/web/tutorials/#add-a-custom-widget) in the Web SDK guide.

The new components can be used to display dashboards on any page with or without the context and widget concept of the group and device dashboards. A demo has been added to the tutorial application. See [Web SDK guide > Overview](/web/#overview) in the Web SDK guide for information on the tutorial application.  
