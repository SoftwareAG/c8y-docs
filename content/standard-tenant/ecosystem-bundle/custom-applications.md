...
4. In the next window, provide a name for the application, an application key to identify the application, a path as part of the URL to invoke the application and finally select an icon for the new applicaiton via the select icon modal. Per default, the values of the original application are provided, extended by a number. If you set the path to the path of the original subscribed application, your own application will overrule the subscribed application.
    {{< c8y-admon-info >}}
