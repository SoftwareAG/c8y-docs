...
Analytics Builder is part of the Streaming Analytics application which runs in Cumulocity IoT, a web-based platform for managing IoT devices \(see also [https://www.cumulocity.com/](https://www.cumulocity.com/)\). It allows you to build analytic models that transform or analyze streaming data in order to generate new data or output events. The models are capable of processing data in real time.
...
Cumulocity IoT is a platform for connecting, monitoring and controlling remote devices. For an overview, see the **Concepts guide** at [https://www.cumulocity.com/guides/](https://www.cumulocity.com/guides/).
...
#### Browsers <!-- {#section_C963B02FA49042F7B9F1ECA129F09F6A .section} -->
...
#### Permissions <!-- {#section_D1CF28D5667349898F73352E477F82F9 .section} -->
...
This is typically achieved by using a global role which has those permissions, and where the role has access to the Analytics Builder application. See the information on the Administration application in the **User guide** at [https://www.cumulocity.com/guides/](https://www.cumulocity.com/guides/) for details on managing permissions.

#### Microservice <!-- {#section_FA53A1C8146A486FA10A93496DF7975A .section} -->
...
![Application switcher showing several applications, including Streaming Analytics](/images/streaming-analytics/analytics-builder/application-switcher.png)

For detailed information on how to use Cumulocity IoT, see [https://www.cumulocity.com/guides/](https://www.cumulocity.com/guides/).

Ask your administrator for the URL that is required to start Cumulocity IoT.

### Language settings

The language in which the user interface of Analytics Builder is shown depends on your user settings in Cumulocity IoT. See the **User guide** at [https://www.cumulocity.com/guides/](https://www.cumulocity.com/guides/) for more information.
...
The steps below require that a device has already been registered in Cumulocity IoT. Preferably, this is a device which is already sending measurement values to Cumulocity IoT. This can be, for example, a smartphone on which the Cumulocity IoT Sensor App has been installed \(see [Cumulocity IoT Sensor App](https://cumulocity.com/guides/users-guide/sensor-app/) in the Cumulocity IoT documentation for detailed information\).
...
#### Step 1: Log on to Cumulocity IoT and switch to Analytics Builder <!-- {#section_7A52FDFDAB9F444C99C1E6F7A761D498 .section} -->

You access Cumulocity IoT via a web browser. See [Getting Started](https://cumulocity.com/guides/users-guide/getting-started/) in the Cumulocity IoT documentation for detailed information.
...
3.  In the top bar of the Cumulocity IoT user interface, click the application switcher button \(<img src="/images/streaming-analytics/analytics-builder/icon-app-switcher.png" alt="Application switcher button" style="max-width:100%">\) and then click **Streaming Analytics**.
...
#### Step 2: Add a new model <!-- {#section_D7BEB4F100424C33965C5CF639985A50 .section} -->
...
#### Step 3: Add the input block <!-- {#section_61E97F8D18264A6A9BB35DF1753EFE67 .section} -->
...
If you need detailed information on the currently selected block, view the block reference in the documentation pane on the right. If the documentation pane is currently not shown, click <img src="/images/streaming-analytics/analytics-builder/icon-docpane-hidden.png" alt="Document icon" style="max-width:100%">.

#### Step 4: Add the block that calculates the mean of the measurement values <!-- {#section_5FF5CD096D6C40F7928B8C72FED3599A .section} -->
...
#### Step 5: Add the output block <!-- {#section_C37B1FC25B4E4D23B9065F6E1F1EF089 .section} -->
...
#### Step 6: Connect the blocks <!-- {#section_8A66303E241247138C15600F1297380F .section} -->
...
#### Step 7: Save the model and go back to the model manager <!-- {#section_A091B8EE6A494F60BD71E5647C559CB8 .section} -->

1.  In the toolbar of the model editor, click <img src="/images/streaming-analytics/analytics-builder/icon-save.png" alt="Save icon" style="max-width:100%"> to save your newly created model.
2.  In the toolbar of the model editor, click <img src="/images/streaming-analytics/analytics-builder/icon-exit.png" alt="Close icon" style="max-width:100%"> to leave the model editor and thus to return to the model manager.
...
{{< /c8y-admon-info>}}
#### Step 8: Activate the model in production mode <!-- {#section_15E1419DDDE04D23AF315D23413C1E69 .section} -->
...
#### Step 9: Go to the Device Management and view the measurements <!-- {#section_3220D8FED25549FBBE26EA77F4BF8C47 .section} -->

To view the measurements that are sent from your active model, you have to switch to Cumulocity IoT's Device Management application. See [Device Management](https://cumulocity.com/guides/users-guide/device-management/) in the Cumulocity IoT documentation for detailed information.

1.  Click the application switcher button \(<img src="/images/streaming-analytics/analytics-builder/icon-app-switcher.png" alt="Application switcher button" style="max-width:100%">\) and then click **Device management**.
...
-   The **Text Substitution** block supports replacement of placeholders. For example, if the input text is “Missing measurements of type: \#\{type\}”, then the `#{type}` value is replaced by the actual `type` of the measurement. See [Text Substitution](re_AnaBui_block_reference_Utilities_TextSubstitution.md) for more details.
...
#### Step 1: Create a new model from a sample { .section}
...
4.  In the toolbar of the model editor, click <img src="/images/streaming-analytics/analytics-builder/icon-save.png" alt="Save icon" style="max-width:100%">.
5.  In the toolbar of the model editor, click <img src="/images/streaming-analytics/analytics-builder/icon-exit.png" alt="Close icon" style="max-width:100%"> to leave the model editor and thus to return to the model manager.

{{< c8y-admon-info>}}
Keep in mind that only saved models are listed on the **Models** tab of the model manager.
{{< /c8y-admon-info>}}

#### Step 2: Create a new instance of the model { .section}
...
#### Step 3: Fill in the template parameter values { .section}
...
#### Step 4: Activate the instance { .section}
...
#### Step 5: Send in the data from your device { .section}
...
#### Step 6: Go to the Device Management and view the alarms <!-- {#section_3220D8FED25549FBBE26EA77F4BF8C47 .section} -->

To view the alarms that are sent from your active instance, you have to switch to Cumulocity IoT's Device Management application. See [Device Management](https://cumulocity.com/guides/users-guide/device-management/) in the Cumulocity IoT documentation for detailed information.

1.  Click the application switcher button \(<img src="/images/streaming-analytics/analytics-builder/icon-app-switcher.png" alt="Application switcher button" style="max-width:100%">\) and then click **Device management**.
2.  In the navigator on the left, click **Devices** and then **All devices**.
3.  Locate your device and click its name to display the device details.
4.  Click **Alarms** on the left.
5.  On the resulting page, check the alarms that are sent from your device. If you have edited your instance as described above, you should see a MINOR alarm after 30 seconds, saying “Missing measurements of type: c8y\_Gyroscope”. See [Working with alarms](https://cumulocity.com/guides/users-guide/device-management/#alarm-monitoring) in the Cumulocity IoT documentation for more information on the **Alarms** tab.
