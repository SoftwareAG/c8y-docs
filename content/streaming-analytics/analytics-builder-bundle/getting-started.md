...
-  On the home screen of the Streaming Analytics application, click the **Open** button that is shown below the Analytics Builder heading.<br>
...
2.  Drag the **Measurement Input** block onto the canvas.<br>
...
4.  From the **Fragment and Series** drop-down list box, select the fragment and series for which the input block is to listen.<br>
...
5.  Select the **Ignore Timestamp** check box.<br>
...
3.  In the block parameter editor, specify a value for **Window Duration \(secs\)**, for example "10".<br>
...
4.  Click **Measurements** on the left. This is a dynamic tab which is only shown when measurements are available for the device.<br>
...
2.  Click the actions menu of the **On missing measurements create alarm** sample and then click **Create model from sample**.<br>
