...
Each row that is shown for an instance has an actions menu \(the three vertical dots that are shown at the very right of the row\) which contains commands for managing the instance \(for example, to delete the instance\).
