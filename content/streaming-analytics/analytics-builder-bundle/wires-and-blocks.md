...
-   **Sample** input and **Output Threshold** parameter
    ...
    Take care to avoid **Output Threshold** values that are too large or too small. If the values are too large, the block does not generate a new output when needed \(unless the **Sample** input is used\). If the values are too small, the block limits how frequently it generates output. If you want to change the values, send a `POST` request to Cumulocity IoT that changes the value for the `minimum_wait_time_secs` key. See [Configuration](/streaming-analytics/analytics-builder/#configuration) for detailed information.
