...
Alarms are created by user applications in the {{< product-c8y-iot >}} tenant (for example, by an analytic model, an activated EPL file, or a smart rule). To learn about alarms in general, refer to [Device management > Working with alarms](/users-guide/device-management/#alarm-monitoring). The Apama-ctrl microservice also generates alarms because it has encountered some problem, so that the user is notified about the situation. The information below is about alarms that are generated by the Apama-ctrl microservice, their causes, consequences and possible ways to resolve them.
...
1. In the Cockpit application. See [Cockpit](/users-guide/cockpit/) for detailed information.
2. In the Administration application, under **Ecosystem** > **Microservices**. Click the Apama-ctrl microservice and then click **Status**.
   See [Administration > Managing and monitoring microservices](/users-guide/administration/#managing-microservices) for detailed information.
...
See [Administration > Viewing audit logs](/users-guide/administration/#audit-logs) and
