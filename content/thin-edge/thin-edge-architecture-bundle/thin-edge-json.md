...
The keys can only have alphanumeric characters, and the `_` (underscore) character but must not start with an underscore. Values can only be numeric. String, Boolean or other JSON object values are not allowed.

### Multi-valued measurements

A multi-valued measurement is a measurement that is comprised of multiple values. Here is the representation of a `three_phase_current` measurement that consists of `L1`, `L2` and `L3` values, representing the current on each phase, see example below.
