...
Configure required parameters for thin-edge.io with [`tedge config set`](/thin-edge/thin-edge-developer-tools/#set):
...
> For example, if we issue [`tedge connect c8y`](/thin-edge/thin-edge-developer-tools/#tedge-connect) without any configuration following advice will be given:
>
> ```shell
> $ tedge connect c8y`
> ...
...
You can upload root certificate via [{{< product-c8y-iot >}} UI](/device-sdk/mqtt/#device-certificates) or with [`tedge cert upload`](/thin-edge/thin-edge-developer-tools/#upload) as described below.
...
To create bridge use [`tedge connect`](/thin-edge/thin-edge-developer-tools/#tedge-connect):
...
To remove existing connection and create new one follow the advice and issue [`tedge disconnect c8y`](/thin-edge/thin-edge-developer-tools/#tedge-disconnect):
...
And now you can issue [`tedge connect c8y`](/thin-edge/thin-edge-developer-tools/#tedge-connect) to create new bridge.
...
To retry start with [`tedge disconnect c8y`](/thin-edge/thin-edge-developer-tools/#tedge-disconnect) removing this bridge:

```shell
sudo tedge disconnect c8y
```

When this is done, issue [`tedge connect c8y`](/thin-edge/thin-edge-developer-tools/#tedge-connect) again.
