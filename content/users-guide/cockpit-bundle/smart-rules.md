...
helpcontent: >-
  ...
  {{< product-c8y-iot >}} includes preset smart rules templates, for example "On measurement threshold create alarm". Since the functionality of the various smart rule types differs, different parameters are required to configure it. See the smart rules collection documentation for details on each smart rule template and its configuration. 
---
