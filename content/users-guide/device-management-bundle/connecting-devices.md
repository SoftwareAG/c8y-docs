...
In this mode any device connected to {{< product-c8y-iot >}} is required to use a security token on establishing the connection and a user is required to enter the same token when accepting the device.
