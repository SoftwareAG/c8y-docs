...
{{< c8y-admon-related >}}
* [Device management library > Device availability](/reference/device-management-library/#device-availability) in the *Reference guide* for the device management library entry on the `c8y_RequiredAvailability`, `c8y_UnavailabilityAlarm`, `c8y_Availability` and `c8y_Connection` fragments used in managed objects.
...
* The [device control API](https://cumulocity.com/api/{{< c8y-current-version >}}/#tag/Operations) for REST API methods concerning operations.
* The [device control API](https://cumulocity.com/api/{{< c8y-current-version >}}/#tag/Bulk-operations) for REST API methods concerning bulk operations.
{{< /c8y-admon-related >}}
