...
With the {{< product-name-1 >}} simulator all aspects of IoT devices can be simulated, as
...
A simulator uses a playlist to simulate messages that the device sends to the {{< product-name-1 >}} platform. A playlist is a series of instructions that the simulator executes one after the other. When the last instruction is reached, the simulator starts again with the first one.
...
The measurement instruction refers to a fragment. Fragments are used to identify capabilities of a managed object. Find more details about fragments in [Sensor Library](https://{{< currently >}}/api/#section/Sensor-library) in the {{< OpenAPI >}}.
