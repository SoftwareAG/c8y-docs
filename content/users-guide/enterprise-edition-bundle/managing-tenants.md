...
When the subtenant is created, it is given an autogenerated ID, which cannot be changed. Also, it is automatically provisioned with first, administrative user ("Administrator's username"). This administrator can create other users and set their permissions. The first user cannot be deleted to prevent you from locking yourself out.
