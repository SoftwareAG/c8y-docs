...
Capturing data from nearby Bluetooth devices and sending them to Cumulocity IoT provides an easy way to connect devices and sensors that may otherwise be difficult or require implementation work.

Cumulocity IoT Sensor App does not only send data to Cumulocity IoT.
You can also send commands to the smartphone directly from the phone dashboard.
...
> **Info:** The Barista.io demo demonstrates the use of connected weight and pressure sensors to brew the perfect coffee. The demo sends measurements from up to 4 connected Bluetooth devices to Cumulocity in real time.
...
There are no limitations within a Cumulocity free trial. 
...
There are two ways of connecting your smartphone to Cumulocity IoT which depend on the version of Cumulocity IoT.
For Cumulocity IoT versions starting with release 10.6.6, you can connect your phone via [QR code](#registration-using-qr-code).
Earlier versions of Cumulocity IoT should follow the steps in [Manual registration](#manual-registration).
...
> **Info:** QR codes not supported by Cumulocity IoT Sensor App will be highlighted using a red region of interest in the Camera view. QR codes from older versions of Cumulocity IoT will be scanned, but it is not possible to connect automatically. Instead you will be forwarded to the **Manual Registration** dialog with your tenant and instance pre-filled. From there you should continue with step 4 in [Manual Registration](#manual-registration).

#### Manual Registration

1.  Using a desktop or laptop computer, open a web browser and log in to your Cumulocity tenant. In the Device Management application, choose **Devices > Registration** from the navigator.

    ![](/images/users-guide/csa2/csa2-device-registration.jpg)

2.  From your smartphone, launch the application and tap **Connect** at the top right of the screen. When the camera opens, tap **Manual Registration**.
...
If your phone has a gyroscope sensor, a 3D rotation widget will depict the current sensor data for your phone's orientation.
...
Your phone's internal sensors, such as its gyroscope, barometer, location and magnetic field, will be shown in cards at the top of the page.
Swipe left and right to see inspect them.
...
> **Info:** Your smartphone will allow you to view sensor data without being connected to Cumulocity IoT. Ensure that your phone is connected to Cumulocity IoT when you wish to have sensor data sent to the server.

_Selecting a card opens the sensor details with a few more details, including when the last measurement was last updated.
...
Use the update time to find out if measurements are received._
...
To connect a device, click the plus at the bottom right of the screen.
...
New devices will be added to the list as the phone discovers them.
...
Bring the device close to your phone, 30cm or less is ideal.

When the Bluetooth device appears in the list, tap **Pair Device** to start pairing.
The Bluetooth device will then be connected to your phone and start sending data to Cumulocity IoT if you are currently connected.

### Configuring, disabling or removing devices

If you wish to make changes to the phone's or a connected Bluetooth device's settings, simply tap the 3 dots at the top right of the card for that device.
...
The phone itself will only have **Disable** and **Edit** options. 

![](/images/users-guide/csa2/csa2-device-options.png)

Tapping **Disconnect** will remove a Bluetooth device from your phone entirely.

Tapping **Disable** will still allow the phone to monitor measurements from this device, but will stop sending sensor measurements Cumulocity IoT.
...
> **Info:** Note that smaller update intervals will give better response times and more complete measurement data, but will also lead to higher power usage.
