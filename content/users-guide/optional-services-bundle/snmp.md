...
Simple Network Management protocol (SNMP) is an application layer protocol, used widely in network management for monitoring network devices. The following image provides a general overview of the SNMP-enabled device integration with Cumulocity:
...
**Mibparser** is an open source, multi-tenant microservice. The procedure to build and deploy the microservice is described in the README.md file

https://bitbucket.org/m2m/c8y-mib-parser/src/develop/
...
7. On successful subscription, the Mibparser microservice will appear under **Applications** > **Subscribed applications** in the subtenant user interface.
...
2. In the Device Management application, click on **Device types > Device protocols** in the left menu.
...
The device protocol mapping helps the agent to know how to deal with incoming data from the SNMP-enabled devices. It basically allows users to configure an OID with Cumulocity object/s such as an alarm, event or measurement. This information is later used by the agent to convert incoming data (say TRAP) to corresponding Cumulocity object/s that are defined in the mapping.
...
4. Under **Functionalities**, switch the toggle button to turn on the mapping for the required Cumulocity model (**Send measurement**, **Raise alarm** and/or **Send event**). Fill in the values for the respective fields and click on **Save** button.
...
The SNMP Agent is a stand-alone Java application that communicates with SNMP-enabled device(s) and the Cumulocity IoT platform. It receives SNMP data from the devices, converts the data to Cumulocity-based objects based on the device protocol mapping, persists the data locally, and forwards the data to Cumulocity. The agent has to be registered (as a device) in Cumulocity before serving the device request.
...
To setup the SNMP agent follow these steps:
...
> **Info**: This upgrade procedure is only for GA releases. If you have installed any previous release prior to GA release, follow the Migration procedure.
...
2. If there are pending messages in the SNMP agent to be sent to the platform, wait for the processing to complete.
3. Once the message processing is complete, stop the agent process (refer to previous version operation manual for the commands to stop).
4. Take backup of the $HOME/.snmp folder, the configuration file in /etc/snmp (if present) and the agent jar file (if jar file is used for starting the agent).
...
7. Delete the snmp-agent device in Cumulocity, that was registered as part of the installation and all its child SNMP devices. This can be done from the user interface or by using REST endpoints.
...
1. In the Device Management application, click on **Devices > Registration** in the left menu.
2. Click on **Register device** button.
3. In the resulting dialog box, select **General device registration**
4. Enter the device ID. The device ID corresponds to the *gateway.identifier* value mentioned in the *snmp-agent-gateway.properties* file.
5. Click on **Next** button to proceed with the device registration and then click on **Complete** button. The device will be shown in the **Device registration** page with the status as WAITING FOR CONNECTION.
6. If the agent process is running with the device ID (as mentioned in the UI), will see an **Accept** button. If the agent is not started, start the agent application.
7. Click on **Accept** button to complete the registration process.
...
3. Enter the **IP range** in the AUTODISCOVERY section and click on **Save** button.
4. Once the changes are saved, **Start autodiscovery** button gets enabled.
5. Click on **Start autodiscovery** button to start auto-discovery once. This creates an operation and the status of the operation can be monitored in the **Control** tab.
6. Once the operation completes, If a new SNMP-enabled device is identified, it will be added to the devices list. Alternatively, added SNMP devices can be seen in the **Child devices** tab.

If you want to run autodiscovery after every interval, enter the interval in the **Scheduled interval** field and click **Save**. When the agent refreshes the configuration data, the scheduled autodiscovery will automatically. You don´t need to click on **Start autodiscovery** button in case of scheduled autodiscovery.
...
* 	if an existing device in the platform (that is within the IP range) is not reachable during the autodiscovery scan.
...
5. Click on **Add** button to add the device. The SNMP device will be listed in the devices list and in the **Child devices** tab.
...
A TRAP is an urgent message sent from the SNMP device to the agent. The SNMP device must send the TRAPs to the agent at the port number defined in `snmp.trapListener.port` in the agent configuration file (default port number is 6671). At the SNMP device side, agent connectivity details has to be configured correctly to send the TRAP to the agent. 

For SNMP v1 and v2c, the community target at the SNMP device side has to be same as in the agent. At the agent side, this is configured in *snmp-agent-gateway.properties* and this should match with the SNMP device. In case of SNMP v3, the authentication and privacy details need to be configured before the SNMP device can send the TRAP to agent.

A TRAP contains a PDU object which contains variables, each containing an OID and a value. If this OID is configured with a mapping in the device protocol assigned to the SNMP device in the platform, corresponding Cumulocity object/s such as alarm/event/measurement will be created in the platform based on the mapping.
...
The SNMP agent provides the capability to poll for SNMP device data by an OID. The device protocol that is configured for the SNMP device, if any of the OIDs have measurement mapping enabled, these OIDs will be polled for the data. If an OID does not contain measurement mapping it will be skipped from polling.
