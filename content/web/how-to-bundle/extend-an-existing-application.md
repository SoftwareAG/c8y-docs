...
As we need to make sure that Angular and angularjs run side by side when running a hybrid app, there are some limitations. The following list tries to explain them:
 ...
 - It is not possible to access the `index.html`: The whole bootstrapping process needs to be handled by Cumulocity to make sure, that all required elements for Angular and angularjs are in place. There is no possibility to change the bootstrapping template, you can only add routes.
 ...
 - Every extension needs to be done via a hook. This is because Angular and angularjs need to use extend the application and the hooks can be used by both.
