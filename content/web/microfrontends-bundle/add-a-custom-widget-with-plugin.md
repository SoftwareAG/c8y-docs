...
See [Add a custom widget to a dashboard > Create the widget components](#2-create-the-widget-components) on how to create a simple widget, what its structure looks like and how to add it to your web app.
The following tutorial focuses on how you can add this widget to a web app using the Module Federation and how this process differs from the previous one.
...
Select the version for which you want to create a sample web app, for example, "1013.72.0 (next)":
...
Select a web app template as the basis for your plugin, for example, "widget-plugin":
...
Navigate to your web app folder and execute `npm install`.
...
- `exports`: Important field. Defines the Angular modules that will be made available by the widget-plugin for the shell web app (see also the *README.md* file):
...
{{< c8y-admon-info >}}
When creating plugins, the custom modules are the backbone of this approach. The exported module is treated as the entry point that links the plugin with the web app, which is referred to as the shell. You can create and export several modules, which have to contain ready-made functionality.
...
To facilitate the process of creating a new plugin, the local server command was extended with a new flag to proxy all requests to the shell web app "Cockpit".
...
We recommend this as the first step in verifying the correctness of the exported module. It facilitates the web app debugging.
After importing your own modules, execute `npm start` to see if the local server starts.

To check the plugin at a later stage, we recommend you to control it locally with various shell web apps, using `npm start -- --shell cockpit`.
...
### 4. Adding a deployed widget to the shell web app

Currently, the views and logic related to Module Federation are hidden behind a beta flag.
To add the uploaded widget-plugin to the dashboard in the Cockpit web app, follow these steps:

- Enable the beta flag in the Administration web app: `https://<yourTenantUrl>/apps/administration?beta=true`.

You should now be able to access the **Packages** tab in **Administration web app > Ecosystem > web apps > Packages**, where you can see the details of your plugin.

- If you already have a custom Cockpit web app, navigate to its **Details** page and then to the **Plugins** tab. Install the widget-plugin.

- If you don't have your own version of the Cockpit web app, navigate to **Administration web app > Ecosystem > web apps** and click **Add web app**. In the resulting dialog, select the option **Duplicate existing web app**. From the list of web apps select **Cockpit (Subscribed)**. Edit the available fields such as **Name**, **Application key**, and **Path**. Use the default values and proceed. Install the `widget-plugin` in the cloned web app.

Your custom widget is now available in your version of the Cockpit web app.
Navigate to the dashboard where the newly added widget is available in the list of widgets to add.

The `widget-plugin` was installed from within the Administration web app. This is the main difference between the regular and the new approach regarding widgets.
Module Federation allows you to add new functionality while the web app is **running** (runtime), whereas the old approach only allowed new functionality to be added before the web app was **built** (compile time).
