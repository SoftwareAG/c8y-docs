---
title: Extend an existing web app and use hooks
layout: redirect
weight: 50
---
...
It is a common use case to extend one of our existing web apps like Cockpit or Device Management.

This recipe explains step by step, how you can extend the Cockpit web app with a custom route and hook this route into the navigator.
First, we will provide some background on what we call a hybrid web app and what the [@c8y/apps](/web/libraries/#application) npm package contains.

### Brief background

The default web apps consist of three web apps which are shipped with our platform by default.
As these web apps are a result of several years of development, the code is mainly based on angularjs.
As we now use Angular for all of our build-in web apps, we needed a solution to serve both frameworks.
The `@c8y/cli` allows scaffolding a default web app which does exactly that.
It uses the Angular upgrade functionality to serve an Angular and angularjs web app at the same time.
...
Due to these limitations, we decided to provide a **pure** Angular empty starter web app which comes without the possibility to integrate angularjs plugins.
This pure version of the web app comes in two flavors:
...
* Extending an existing hybrid web app.
* Building a pure Angular web app with Angular CLI.
* Building it with @c8y/cli.

Which one to choose heavily depends on the web app you want to build.
For example, if you want a web app that just follows the look and feel of the platform but want to use special dependencies for certain scenarios, for example, the Material framework, you are best set with the pure Angular CLI solution.

The most common use case is the extension of a hybrid web app, which we will cover in this recipe.
...
As we need to make sure that Angular and angularjs run side by side when running a hybrid web app, there are some limitations:
 ...
 - As the services need to be loaded first, you can also not inject any service in the root web app module. You need to provide them on a route or as `providedIn: root` at the deceleration of the service.
 ...
 - Every extension needs to be done via a hook. This is because Angular and angularjs are needed in hybrid web apps and the hooks can be used by both.
...
Now that you know the limitations you can start to extend the first web app and develop your first extension hook.
To do so, you must scaffold a hybrid web app.
`@c8y/apps` is a package which contains the default web apps and their minimum setup.
The `c8ycli` uses this package every time you initialize a web app with the `new` command.
The next section will explain the scaffolding process and how to extend a hybrid web app step by step.
...
When you now spin up the web app with the `c8ycli server` command and navigate to the URL by adding the right hash to it (`http://localhost:9000/apps/cockpit/#/hello`) you should see the custom component.
...
![The extended Cockpit web app](/images/web-sdk/route-extension-hook.png)

Note that the property `priority` of the `NavigatorNode` interface defines in which order the nodes are shown.

The `hello.component.ts` is now like a blank canvas inside the Cockpit web app.
You can implement any kind of feature you need, while the given functionality of the Cockpit is not affected.

### Conclusion

A hybrid web app is limited because of its angularjs and Angular integration.
However, the hook concept and a custom route allow for additions to existing hybrid web apps.
They are a powerful tool to extend the build-in web apps.
Sometimes additional features are needed and a pure Angular web app is a better fit.
