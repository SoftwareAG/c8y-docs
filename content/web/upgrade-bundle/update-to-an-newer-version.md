...
Therefore, a new web app needs to be built and deployed to the platform.
...
If you decide not to use an SCM, backup your web app before running the update.
...
So for example if your current working directory is a web app called "my-cockpit" based on {{< product-c8y-iot >}}'s Cockpit web app and you want to update to version 10.6.2.0, you must run the following command:
...
The command simply copies over the files that are used for building a new web app in the particular version.
The following files are currently overwritten:

 - `app.module.ts`: The Angular module imports which might be aligned by your web app to import custom Angular modules.
 ...
 - `package.json`: The npm dependencies, web app options and names are stored in this file. This is very likely changed, for example, when different options or a different dependency are used.
...
After this, start the web app with `npm start`. After login you can check the current UI version by clicking on you username.

If everything worked as expected, you can now deploy your web app by running the command`npm run deploy`.
