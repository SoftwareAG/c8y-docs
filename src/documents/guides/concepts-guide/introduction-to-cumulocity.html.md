...
* Rapid customization of the above through [Cumulocity Event Language](/guides/concepts-guide/real-time-processing-in-cumulocity.html) rules and [Cumulocity SmartApps](/guides/concepts-guide/developing-m2m-applications.html).
...
Through the [Sensor Library](/guides/reference-guide/sensor-library.html), common sensor and control types are correctly rendered regardless of the device that produces the sensor data.
...
* Use [Cumulocity Event Language](/guides/concepts-guide/real-time-processing-in-cumulocity.html) to implement real-time business rules. For example, get an email when critical events happen, or trigger automated actions on devices in that case.
...
* The general technical concepts behind Cumulocity are described in [Cumulocity's domain model](/guides/concepts-guide/modeling-and-managing-m2m-assets.html).
* Concepts related to interfacing devices and other IT systems with Cumulocity are described in [Interfacing with M2M data sources](/guides/concepts-guide/interfacing-with-m2m-data-sources.html).
* Customization concepts of Cumulocity are described in [Real-time processing in Cumulocity](/guides/concepts-guide/real-time-processing-in-cumulocity.html) and [Hosting applications](/guides/concepts-guide/hosting.html).
* Security concepts are described in [Securing M2M applications](/guides/concepts-guide/securing-m2m-applications.html).
