...
**Alarm suppression** If the source device is in [maintenance](/guides/reference-guide/device-management) mode, the alarm is not created and not reported to the Cumulocity event processing engine.
...
|status|String|1|The status of the alarm: ACTIVE, ACKNOWLEDGED or CLEARED. Must be upper-case.|POST: Mandatory PUT: Optional|
