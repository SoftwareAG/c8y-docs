...
The various connection states are illustrated in the image below. "Maintenance mode" is a special state indicating that the device is currently being maintained and is not monitored. While a device is being maintained, no alarms for that device are raised. You can enable maintenance mode by clicking "Turn on maintenance".
