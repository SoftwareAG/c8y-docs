...
* A microservice must be stateless, i.e. it must contain only ephemeral state. The reason is that a microservice must be able to survive (random) restarts due to hardware (server failure) and operations reasons (upgrade, migration).
...
* All log information have to be sent to standard output in order to be captured and persisted by the infrastructure.
