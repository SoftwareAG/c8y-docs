...
Cumulocity LWM2M allows the set of Custom Actions to be extended using Decoder Microservices. A decoder decoder microservice is an ordinary Cumulocity Microservice that implements a simple decoder interface. The LWM2M Agent calls this microservice for decoding data in a customer-specific way. We are providing an according example how to write such a decoder microservice in our public ![Bitbucket repository.](https://bitbucket.org/m2m/cumulocity-examples) 
