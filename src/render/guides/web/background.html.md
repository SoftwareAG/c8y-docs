...
We are about to launch a very important milestone in the UI stack of the Cumulocity platform - moving to the next generation. Therefore we would like to provide some background information on how the stack and the build has evolved over time to better understand why some parts are designed the way they work today.
...
There still is an API ```applications/<id>/plugins``` but it is not used any longer.
...
The loading of the modules was entirely dynamic. We served a very basic index.html that was the same for all applications, logged in the user with just a small jquery (which has constantly grown over time) and only after authentication we fetched the application information and loaded the appropriate plugins from the respective applications. So after login we had something like this:
...
There was no hosting of files. We committed the build to a folder and proxied the requests to the bitbucket raw URLs (https://raw.bitbucket.com/m2m/cumulocity-ui/build).
...
In the beginning we also loaded the 3rd party dependencies through the CDN jsdelivr.
...
3rd party developers would now need to get hold of our plugins to develop and build their apps. So we start deploying a node package to http://resources.cumulocity.com/webapps/ui/<version>.tgz. And it would be installed via npm. These packages would contain the same built versions that is used in the builtin apps with minified code and dependencies bundled.
...
The whole process was focused on enablid the complete stack of modern web dev and try to remove as much specificity from the process as possible.

One big change is that so far, modules (or plugins) where always represented during development and after build, so they could easily be composed by adding or removing a script. As we are now bundling everyhing, the modules can only be put together during development so the possibility to edit an app from the UI dissappears.
